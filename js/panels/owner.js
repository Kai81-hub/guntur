...
window.GP_OWNER_PANEL = {
  role:"owner",
  async init(){ if(!requireRole(["admin","owner"])) return; bindLogout(); $("property-form")?.addEventListener("submit",e=>this.save(e)); await this.load(); },
  async save(e){
    e.preventDefault(); const status=$("property-status"); const data=Object.fromEntries(new FormData(e.target).entries());
    try{ await add(tbl().properties,{...data,approval_status:"pending",user_phone:phone(),seller_type:this.role,contact_phone:data.contact_phone||phone()}); if(status)status.textContent="Property submitted for approval."; e.target.reset(); this.load(); }
  ...
  async load(){
    ...
    }catch(err){ window.GP_ERROR_LOGGER?.log(err,"owner.js"); toast(err.message,"error"); }
  },
...
document.addEventListener("DOMContentLoaded",()=>window.GP_OWNER_PANEL.init());
